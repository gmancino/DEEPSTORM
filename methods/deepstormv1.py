            ...
            if i % self.report == 0:
                ...
                if rank == 0:
                    ...
                    print("{:<10} | {:<7} | {:<13} | {:<15} | {:<15} | {:<15} | {:<15} | {:<12} | {:<6}".format(i,
                                                                     round((i * 2 * self.mini_batch) / (self.data_loader.dataset.data.shape[0] // size), 2),
                                                                     round(total, 4),
...
if __name__=='__main__':
    ...
    # Parse user input
    parser = argparse.ArgumentParser(description='Testing proposed method (v1-SG) on problems from paper.')
    ...
    if rank == 0:
        opening_statement = f' DEEPSTORM v1-SG on {args.data} '
        print(f"\n{'#' * 75}")
    ...
    solver = DEEPSTORMv1(algo_params, mixing_matrix, train_loader, init_weights)
    algo_time = solver.solve(args.updates, optimality_loader, test_loader)

    # Save the information
    method = 'deepstormv1-sg'
