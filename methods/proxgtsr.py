...
class ProxGTSR:
            ...
            if i % self.report == 0:
                ...
                if rank == 0:
                    ...
                    print("{:<10} | {:<7} | {:<13} | {:<15} | {:<15} | {:<15} | {:<15} | {:<12} | {:<6}".format(i,
                                                                     round((i * self.mini_batch - (i // self.full_grad) * self.mini_batch + i // self.full_grad * (self.data_loader.dataset.data.shape[0] // size)) / (self.data_loader.dataset.data.shape[0] // size), 2),
                                                                     round(total, 4),
    ...
    def get_full_grad(self, current_weights, loader):
        ...
        for batch_idx, (data, target) in enumerate(loader):
            ...
            grads = [grads[ind] + (len(target) // ((self.data_loader.dataset.data.shape[0]) // size)) * p.grad.data.detach().clone() for ind, p in enumerate(self.model.parameters())]
...
if __name__=='__main__':
    ...
    parser.add_argument('--full_grad', type=int, default=64, help='When to compute full gradient.')
    ...
    if rank == 0:
        numpy.savetxt(
            f'{path}/test_loss_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.testing_loss, fmt='%.16f')
        numpy.savetxt(
            f'{path}/test_acc_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.testing_accuracy, fmt='%.16f')
        numpy.savetxt(
            f'{path}/train_loss_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.training_loss, fmt='%.16f')
        numpy.savetxt(
            f'{path}/train_acc_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.training_accuracy, fmt='%.16f')
        numpy.savetxt(
            f'{path}/test_loss_local_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.testing_loss_local, fmt='%.16f')
        numpy.savetxt(
            f'{path}/test_acc_local_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.testing_accuracy_local, fmt='%.16f')
        numpy.savetxt(
            f'{path}/train_loss_local_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.training_loss_local, fmt='%.16f')
        numpy.savetxt(
            f'{path}/train_acc_local_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.training_accuracy_local, fmt='%.16f')
        numpy.savetxt(
            f'{path}/total_opt_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.total_optimality, fmt='%.16f')
        numpy.savetxt(
            f'{path}/consensus_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.consensus_violation, fmt='%.16f')
        numpy.savetxt(
            f'{path}/norm_hist_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.norm_hist, fmt='%.16f')
        numpy.savetxt(
            f'{path}/iterate_hist_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.iterate_norm_hist, fmt='%.16f')
        numpy.savetxt(
            f'{path}/total_time_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.total_time, fmt='%.16f')
        numpy.savetxt(
            f'{path}/comm_time_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.communication_time, fmt='%.16f')
        numpy.savetxt(
            f'{path}/comp_time_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.compute_time, fmt='%.16f')
        numpy.savetxt(
            f'{path}/nnz_at_avg_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
            solver.nnz_at_avg, fmt='%.16f')
        numpy.savetxt(
            f'{path}/avg_nnz_lr{args.lr}_fullgrad{args.full_grad}_l1{args.l1}.txt',
