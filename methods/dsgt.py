...
"""
Test the decentralized stochastic gradient tracking
...
if __name__=='__main__':
    ...
    # Parse user input
    parser = argparse.ArgumentParser(description='Testing DSGT on problems from paper.')
    ...
    if rank == 0:
        opening_statement = f' DSGT on {args.data} '
        print(f"\n{'#' * 75}")
    ...
    # Declare and train!
    algo_params = {'lr': args.lr, 'mini_batch': args.mini_batch, 'report': args.report,
                   'step_type': args.step_type, 'l1': args.l1}
    solver = DSGT(algo_params, mixing_matrix, train_loader, init_weights)
    ...
    if rank == 0:
        numpy.savetxt(
            f'{path}/test_loss_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.testing_loss, fmt='%.16f')
        numpy.savetxt(
            f'{path}/test_acc_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.testing_accuracy, fmt='%.16f')
        numpy.savetxt(
            f'{path}/train_loss_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.training_loss, fmt='%.16f')
        numpy.savetxt(
            f'{path}/train_acc_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.training_accuracy, fmt='%.16f')
        numpy.savetxt(
            f'{path}/test_loss_local_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.testing_loss_local, fmt='%.16f')
        numpy.savetxt(
            f'{path}/test_acc_local_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.testing_accuracy_local, fmt='%.16f')
        numpy.savetxt(
            f'{path}/train_loss_local_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.training_loss_local, fmt='%.16f')
        numpy.savetxt(
            f'{path}/train_acc_local_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.training_accuracy_local, fmt='%.16f')
        numpy.savetxt(
            f'{path}/total_opt_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.total_optimality, fmt='%.16f')
        numpy.savetxt(
            f'{path}/consensus_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.consensus_violation, fmt='%.16f')
        numpy.savetxt(
            f'{path}/norm_hist_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.norm_hist, fmt='%.16f')
        numpy.savetxt(
            f'{path}/iterate_hist_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.iterate_norm_hist, fmt='%.16f')
        numpy.savetxt(
            f'{path}/total_time_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.total_time, fmt='%.16f')
        numpy.savetxt(
            f'{path}/comm_time_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.communication_time, fmt='%.16f')
        numpy.savetxt(
            f'{path}/comp_time_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.compute_time, fmt='%.16f')
        numpy.savetxt(
            f'{path}/nnz_at_avg_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
            solver.nnz_at_avg, fmt='%.16f')
        numpy.savetxt(
            f'{path}/avg_nnz_lr{args.lr}_step{args.step_type}_l1{args.l1}.txt',
